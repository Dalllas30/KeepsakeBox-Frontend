...
@Component({
  ...
  styleUrl: './patient-statistics.component.css'
})
export class PatientStatisticsComponent implements OnInit, AfterViewInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    // Pre-populate pwDArray with the current patient pre-selected
    const currentPatient = this.patientService.getCurrentPatient();
    if (currentPatient) {
      this.pwDArray.push({ id: currentPatient.id, name: currentPatient.name });
      this.pwDSelected = currentPatient.id;
    } else {
      // Fallback: show all patients if no current patient in context
  ...
  async getPwD(): Promise<void> {
    ...
    // Always include current patient at the top if available
    if (currentPatient) {
      this.pwDArray.push({ id: currentPatient.id, name: currentPatient.name });
    }
  ...
  async createChartNPNS(chartName: string, config: any): Promise<void> {
    ...
    if (canvas == null) {
      ...
      this.chartNPNS = new Chart(
        ...
        this.pwDSelected === 'Todos' || this.pwDSelected === 'All'
          ? config
          : this.getConfigChart()
      );
    } else {
  ...
  async createChartMostCoveredThemes(chartName: string, config: any): Promise<void> {
  ...
  async getLabelsMCT(): Promise<void> {
    ...
    for (let index = 0; index < 4; index++) {
      for (let p = 0; p < this.imagesCategoryPerSession.length; p++) {
        if (this.labelsMCT[index] === this.imagesCategoryPerSession[p].cat) {
      ...
      catPacients.sort((a, b) => (countPatients[a] > countPatients[b] ? -1 : 1));
      ...
      for (let pp = 0; pp < catPacients.length; pp++) {
        ...
        if (targetArray.length < 5 && !targetArray.includes(catPacients[pp])) {
          targetArray.push(catPacients[pp]);
        } else if (targetArray.length >= 5) {
          break;
        }
      }
            ...
            callbacks: {
              afterLabel: (context: any) => {
                ...
                for (let axis = 0; axis < 4; axis++) {
                  if (context.parsed.x !== null && context.parsed.y === axis) {
                    for (let d = 0; d < dataAllSessionsMCT[axis].patients.length; d++) {
                      if (dataAllSessionsMCT[axis].patients.length > 1) {
                        if (d === dataAllSessionsMCT[axis].patients.length - 1) {
                          label += 'e ' + dataAllSessionsMCT[axis].patients[d];
                        } else if (d === dataAllSessionsMCT[axis].patients.length - 2) {
  ...
  async getConfigChartAllPatients(): Promise<void> {
    const dataAllSessions = [
      { patients: this.aferLabelPatientPositive, count: this.countPositive },
      { patients: this.aferLabelPatientNeutral,  count: this.countNeutral  },
      { patients: this.aferLabelPatientNegative, count: this.countNegative }
    ];
    this.configSessionsPANAllPatients = {
      plugins: [ChartDataLabels],
      type: 'bar',
      data: {
        labels: [this.translate()[0], this.translate()[1], this.translate()[2]],
        datasets: [
          {
      ...
      options: {
        parsing: { xAxisKey: 'count', yAxisKey: 'count' },
        plugins: {
          datalabels: {
            color: '#FFFFFF',
            formatter: (context: any) => JSON.stringify(context.count)
          },
          legend: { display: false },
          tooltip: {
            callbacks: {
              afterLabel: (context: any) => {
                ...
                for (let axis = 0; axis < 3; axis++) {
                  if (context.parsed.y !== null && context.parsed.x === axis) {
                    for (let d = 0; d < dataAllSessions[axis].patients.length; d++) {
                      if (dataAllSessions[axis].patients.length > 1) {
                        if (d === dataAllSessions[axis].patients.length - 1) {
                          label += 'e ' + dataAllSessions[axis].patients[d];
                        } else if (d === dataAllSessions[axis].patients.length - 2) {
  ...
  async countDataChart(): Promise<void> {
    ...
    for (let index = 0; index < patientPositive.length; index++) {
      ...
      countP[element] = (countP[element] ?? 0) + 1;
    }
    allPatientPositive.sort((a, b) => (countP[a] > countP[b] ? -1 : 1));
    for (let pp = 0; pp < 5; pp++) {
      if (allPatientPositive[pp] != null) this.aferLabelPatientPositive.push(allPatientPositive[pp]);
    }
    ...
    for (let index = 0; index < patientNeutral.length; index++) {
      ...
      countNeu[element] = (countNeu[element] ?? 0) + 1;
    }
    allPatientNeutral.sort((a, b) => (countNeu[a] > countNeu[b] ? -1 : 1));
    for (let pp = 0; pp < 5; pp++) {
      if (allPatientNeutral[pp] != null) this.aferLabelPatientNeutral.push(allPatientNeutral[pp]);
    }
    ...
    for (let index = 0; index < patientNegative.length; index++) {
      ...
      countNeg[element] = (countNeg[element] ?? 0) + 1;
    }
    allPatientNegative.sort((a, b) => (countNeg[a] > countNeg[b] ? -1 : 1));
    for (let pp = 0; pp < 5; pp++) {
      if (allPatientNegative[pp] != null) this.aferLabelPatientNegative.push(allPatientNegative[pp]);
    }
  }

  async showChart(filterPatient: any, filtersMonth: any, filtersYear: any): Promise<void> {
    this.caregiverSessions = await this.sessionService.getSessionListByDateCaregiver(
      this.authenticationService.getCurrentCaregiverToken()!,
      filtersMonth,
      filtersYear,
    ...
    if (this.caregiverSessions.length !== 0) {
      ...
      if (this.chartMTC) this.chartMTC.destroy();
      ...
      if (this.open) {
        ...
        await this.countDataChart();
        await this.getConfigChartAllPatients();
        if (this.chartNPNS) await this.chartNPNS.destroy();
        this.createChartNPNS('stackedBarChart', this.configSessionsPANAllPatients);
  ...
  averageDurationSessions(session: Session[]): void {
    ...
    for (let s = 0; s < session.length; s++) {
      ...
      totalDuration = date.toTimeString().replace(/.*(\d{2}:\d{2}:\d{2}).*/, '$1');
    }

    const d = totalDuration.split(':');
    const convertSecondsToTime =
      (+d[0] * 60 * 60 + +d[1] * 60 + +d[2]) / session.length;
    ...
    const finalHour =
      hour === '00' ? '' :
      hour === '01' ? this.averageDurationsSessions.substring(1, 2) + this.translate()[6] + ' ' :
      this.averageDurationsSessions.substring(0, 1) === '0'
        ? this.averageDurationsSessions.substring(1, 2) + this.translate()[7] + ' '
        : hour + this.translate()[7];

    const finalMinute =
      minute === '00' ? '' :
      minute === '01' ? this.averageDurationsSessions.substring(4, 5) + this.translate()[8] :
      this.averageDurationsSessions.substring(3, 4) === '0'
        ? this.averageDurationsSessions.substring(4, 5) + this.translate()[9]
        : minute + this.translate()[9];

    const finalSecond =
      second === '00' ? '' :
      second === '01' ? this.averageDurationsSessions.substring(7, 8) + this.translate()[10] :
      this.averageDurationsSessions.substring(6, 7) === '0'
        ? this.averageDurationsSessions.substring(7, 8) + this.translate()[11]
        : second + this.translate()[11];

    this.finalAverageDurationsSessions = finalHour + finalMinute + finalSecond;
  }
